...
function validReport(report) {
  if (
    ...
    !Number.isInteger(report.checked) ||
    ...
    report.failures.length !== report.failed
  )
...
function filteredFailures(report) {
  validReport(report);
  const candidates = report.failures
    .map((failure) => {
      if (
        !failure ||
        typeof failure !== "object" ||
        ["key", "name", "date", "filename", "url", "status", "reason"].some(
          (field) => typeof failure[field] !== "string",
        )
      ) {
        throw new Error("Malformed report");
      }
...
function validState(state) {
  return (
    state &&
    typeof state === "object" &&
    (state.status === "healthy" || state.status === "failing") &&
    Array.isArray(state.failureKeys) &&
    state.failureKeys.every((key) => typeof key === "string") &&
    Number.isInteger(state.permissionCount) &&
    Number.isInteger(state.invalidCount)
  );
}
...
async function saveState(path, state) {
  const temporary = join(
    dirname(path),
    `.${stateFileName(path)}.${randomUUID()}.tmp`,
  );
  await writeFile(temporary, `${JSON.stringify(state)}\n`, { mode: 0o600 });
  await rename(temporary, path);
}
function stateFileName(path) {
...
async function send(payload, webhook, fetchImpl = globalThis.fetch) {
  ...
  try {
    ...
    throw error instanceof Error && error.message === "Discord request failed"
      ? error
      : new Error("Discord request failed");
  } finally {
    clearTimeout(timer);
  }
}
...
export async function processNotification({
  ...
  statePath,
  workflowUrl,
  webhook = process.env.DISCORD_GALLERY_WEBHOOK_URL,
  ...
  try {
    ...
    if (!validState(previous)) previous = null;
  } catch {
  ...
  if (transition) {
    ...
    await send(
      makePayload({
        transition,
        failures,
        previous: previous ?? { permissionCount: 0, invalidCount: 0 },
        workflowUrl,
      }),
      webhook,
      fetchImpl,
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    ...
    await processNotification({
      report: await readJson(args.report),
